...
if not df_m.empty:
    # التأكد من تنظيف أسماء الأعمدة من أي فراغات مخفية
    df_m.columns = [c.strip() for c in df_m.columns]
    # إنشاء عمود الترتيب من العمود الجديد v_date
    df_m['v_date_dt'] = pd.to_datetime(df_m['v_date'], errors='coerce')
    df_m['amount_num'] = df_m['amount'].apply(to_num)
else:
    # في حالة الشيت فارغ تماماً، ننشئ أعمدة فارغة لمنع انهيار الكود
    df_m = pd.DataFrame(columns=['name', 'v_date', 'v_date_dt', 'amount_num'])
if not df_exp.empty:
    ...
    if menu == "إضافة عميل جديد":
        ...
        for area, group in filtered.groupby('area'):
            ...
            for _, r in group.iterrows():
                with st.expander(f"👤 {r['name']} | 📞 {r['phone']}"):
                    ...
                    # 1. فلترة البيانات أولاً
                    cust_hist = df_m[df_m['name'] == r['name']].copy()

                    # 2. الترتيب فقط إذا كان هناك بيانات والعمود موجود
                    if not cust_hist.empty and 'v_date_dt' in cust_hist.columns:
                        # التأكد من حذف أي قيم فارغة في عمود التاريخ قبل الترتيب
                        cust_hist = cust_hist.dropna(subset=['v_date_dt'])
                        cust_hist = cust_hist.sort_values('v_date_dt', ascending=False)

                    # 3. حساب موعد الزيارة القادمة
        ...
        for d in days_to_show:
            ...
            for _, cust in df_c[df_c['status'] == "نشط"].iterrows():
                # ترتيب تصاعدي لمعرفة أول/آخر زيارة
                temp_hist = df_m[df_m['name'] == cust['name']].copy()
                last_m_all = temp_hist.sort_values('v_date_dt', ascending=True) if not temp_hist.empty else temp_hist
                    ...
                    if next_v == d or (next_v < today and d == days_to_show[0]):
                        with st.expander(f"👤 {cust['name']} | 📍 {cust['area']} | 📞 {cust['phone']}"):
                            ...
                            if not cust_hist.empty:
                                ...
                                show_cols = ['v_date'] + check_cols + ['amount', 'notes']
                                st.dataframe(display_hist[show_cols], use_container_width=True, hide_index=True)
