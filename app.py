...
@st.cache_data(ttl=300) # زيادة الوقت لزيادة السرعة
...
class CustomerPDF(FPDF):
    def header(self):
        try: self.image("logo.png", 10, 8, 45) # اللوجو فوق عاليسار
        except: pass
    ...
    def footer(self):
        ...
        # اسم الشركة ورقم التليفون بخط كبير في الفوتر
        self.cell(0, 10, 'Healthy Water - Contact: 01286609535', 0, 0, 'C')

def generate_pdf(row, df_m):
    pdf = CustomerPDF(orientation='L', unit='mm', format='A4')
    pdf.add_page()
    pdf.set_font("Arial", 'B', 12)
    
    # بيانات العميل الأساسية
    pdf.cell(0, 10, f"Name: {row['name']}", ln=True)
    pdf.cell(0, 10, f"Phone: {row.get('phone','')}", ln=True)
    pdf.cell(0, 10, f"Area: {row.get('area','')}", ln=True)
    pdf.ln(5)
    ...
    # معالجة التواريخ والترتيب
    df_m['dt_temp'] = pd.to_datetime(df_m['visit_date'], errors='coerce')
    sorted_history = df_m.sort_values(by='dt_temp', ascending=False)
    
    pdf.set_font("Arial", '', 10)
    for _, m in sorted_history.iterrows():
        pdf.cell(31, 10, str(m.get('visit_date',''))[:10], 1, 0, 'C')
        for f in ['P1','P2','P3','membrane','post_carbon','Calcite','infrared']:
            val = "V" if format_to_check(m.get(f,'')) == "✓" else "X"
            pdf.cell(31, 10, val, 1, 0, 'C')
...
st.markdown("""
    ...
    .contact-card { background: #fff; padding: 20px; border-radius: 15px; border: 1px solid #ddd; text-align: center; margin-bottom: 20px; }
    </style>
...
if st.session_state.auth == "admin":
    ...
    st.sidebar.markdown('<a href="tel:01286609535" class="call-btn">📞 اتصل فون</a>', unsafe_allow_html=True)
...
if menu in ["بيانات العملاء", "بروفايلي"]:
    ...
    if st.session_state.auth == "customer":
        st.markdown("""<div class="contact-card"><h3>لطلب صيانة أو استفسار تواصل معنا</h3>
        <a href="tel:01286609535" class="call-btn">01286609535</a>
        <a href="https://wa.me/201286609535" class="wa-btn">راسلنا واتساب</a></div>""", unsafe_allow_html=True)
    ...
    for r in data_to_show:
        ...
        with st.expander("فتح التفاصيل الكاملة"):
            ...
            with col1:
                ...
                # أزرار الاتصال (دعم الأرقام المتعددة)
            ...
            with col2:
                ...
                if not history.empty:
                    ...
                    st.download_button(f"📥 تحميل PDF ({r['name']})", pdf_data, f"{r['name']}_Report.pdf", "application/pdf")
