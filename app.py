...
# --- 4. إدارة العملاء وتواصل سريع ---
    ...
    if not df.empty:
        ...
        if search:
            df = df[df['name'].str.contains(search, na=False) | df['area'].str.contains(search, na=False)]
...
# --- 5. تسجيل صيانة (Checkboxes) ---
    ...
    if not df_c.empty:
        ...
        with st.form("maint_form"):
            ...
            next_date = st.date_input("ميعاد الزيارة القادمة (تنبيه)", datetime.now() + timedelta(days=90))
...
# --- 6. جدول الأسبوع ---
    ...
    if not df_m.empty:
        ...
        if not upcoming.empty:
            st.dataframe(upcoming[['next_visit', 'name', 'others', 'amount']], use_container_width=True)
