...
# --- 2. نظام الألوان المطور (لإصلاح مشكلة عدم التغير) ---
def get_status_color(next_date, status):
    ...
    elif diff < 0: return "#dc3545" # أحمر
    else: return "#f0f2f6"
...
if menu == "بيانات العملاء":
    ...
    for idx, r in filtered_df.iterrows():
        ...
        st.markdown(f"""
            <div style="padding:12px; border-radius:8px; margin-bottom:10px; background-color:#ffffff; border-right:15px solid {status_color}; border-left:1px solid #ddd; border-top:1px solid #ddd; border-bottom:1px solid #ddd; box-shadow: 2px 2px 5px rgba(0,0,0,0.05);">
                <h4 style="margin:0; color:#333;">👤 {r['name']} 
        ...
        with st.expander("فتح التفاصيل وسجل الصيانات"):
            ...
            with c1:
                ...
                st.write("---")
                # أيقونات التعديل والحذف اللي كانت مكتوبة None
            ...
            with c2:
                ...
                if not history.empty:
                    ...
                    # عرض الجدول مع أيقونات صغيرة للتعديل والحذف بجانب كل زيارة
                    ...
                    st.table(h_display)
                    st.download_button("📥 تحميل PDF", generate_safe_pdf(r, df_m), f"{r['name']}.pdf", key=f"pdf_down_{idx}")
    ...
    for _, r in df_c.iterrows():
        ...
        if last_v:
            ...
            if pd.notnull(spec_d): n_date = spec_d.date()
                ...
                cycle = int(r.get('maintenance_cycle', 3))
                n_date = (last_v['v_date_dt'] + timedelta(days=cycle * 30)).date()
            upcoming.append({"name": r['name'], "date": n_date, "area": r.get('area','')})
    
    sched_df = pd.DataFrame(upcoming)
    today = datetime.now().date()
    # عرض 7 أيام
    for i in range(7):
        curr_day = today + timedelta(days=i)
        st.subheader(f"{get_arabic_day(curr_day)} ({curr_day})")
        day_clients = sched_df[sched_df['date'] == curr_day]
        if not day_clients.empty:
            for _, c_row in day_clients.iterrows():
                st.markdown(f"🔹 **[{c_row['name']}](بيانات_العملاء#{c_row['name'].replace(' ','_')})** | 📍 {c_row['area']}")
        else:
    ...
    with st.form("complete_m_form"):
        ...
        st.write("---")
        # الخانات اللي كانت ناقصة (أخرى وملاحظات)
